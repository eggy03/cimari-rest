package io.github.eggy03.cimari.rest.controller.storage;

import io.github.eggy03.cimari.rest.entity.storage.Win32DiskDrive;
import io.github.eggy03.cimari.rest.service.storage.Win32DiskDriveService;
import jakarta.ws.rs.GET;
import jakarta.ws.rs.Path;
import jakarta.ws.rs.Produces;
import jakarta.ws.rs.core.MediaType;
import lombok.RequiredArgsConstructor;
import org.eclipse.microprofile.openapi.annotations.Operation;
import org.eclipse.microprofile.openapi.annotations.responses.APIResponse;
import org.eclipse.microprofile.openapi.annotations.tags.Tag;

import java.util.List;

@Path("api/v1/disk/drive")
@Produces(MediaType.APPLICATION_JSON)
@RequiredArgsConstructor
@Tag(name = "Storage")
public class Win32DiskDriveController {

    private final Win32DiskDriveService service;

    @GET
    @Operation(summary = "List of Win32_DiskDrive objects")
    @APIResponse(responseCode = "200", description = "Success")
    public List<Win32DiskDrive> getAll() {
        return service.get(15);
    }
}
